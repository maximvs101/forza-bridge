...
from smoothing import (NON_LISSABLES, SUFFIXE, Smoother, formate_reglages,
                       parse_reglages)


class TestLectureDesReglages(unittest.TestCase):
    def test_specification_simple(self):
        self.assertEqual(parse_reglages("speed_kmh=0.15"), {"speed_kmh": 0.15})

    def test_plusieurs_canaux(self):
        lu = parse_reglages("speed_kmh=0.15, slip_max=0.05 ; throttle=0.2")
    ...
    def test_entrees_invalides_ignorees(self):
        ...
        for texte in ("", "n'importe quoi", "speed_kmh", "speed_kmh=abc",
            ...
            with self.subTest(texte=texte):
                self.assertEqual(parse_reglages(texte), {})

    def test_canal_non_lissable_refuse(self):
        self.assertEqual(parse_reglages("gear=0.2"), {})
        self.assertEqual(parse_reglages("car_ordinal=0.5"), {})

    def test_aller_retour(self):
        reglages = {"speed_kmh": 0.15, "throttle": 0.05}
        self.assertEqual(parse_reglages(formate_reglages(reglages)), reglages)
...
class TestNonAlteration(unittest.TestCase):
    ...
    def test_le_lisse_est_un_canal_distinct(self):
        ...
        self.assertIn("speed_kmh" + SUFFIXE, sortie)
        self.assertNotEqual(sortie["speed_kmh" + SUFFIXE], sortie["speed_kmh"])
    ...
    def test_pics_conserves_dans_le_brut(self):
        ...
        for i in range(120):
            ...
            lisses.append(sortie["speed_kmh" + SUFFIXE])
    ...
    def test_canaux_produits_annonces(self):
        s = Smoother({"speed_kmh": 0.2, "slip_max": 0.1})
        self.assertEqual(s.canaux_produits, ["slip_max_smooth", "speed_kmh_smooth"])

    def test_aucun_canal_produit_sans_reglage(self):
        self.assertEqual(Smoother().canaux_produits, [])
...
class TestFiltre(unittest.TestCase):
    ...
    def test_premiere_valeur_adoptee_telle_quelle(self):
        ...
        self.assertEqual(sortie["speed_kmh" + SUFFIXE], 100.0)
    ...
    def test_convergence_vers_la_valeur_cible(self):
        ...
        self.assertAlmostEqual(sortie["speed_kmh" + SUFFIXE], 100.0, places=3)
    ...
    def test_retard_reel_avant_convergence(self):
        ...
        self.assertLess(sortie["speed_kmh" + SUFFIXE], 20.0)
    ...
    def test_constante_de_temps_respectee(self):
        ...
        self.assertAlmostEqual(sortie["speed_kmh" + SUFFIXE], attendu, delta=1.0)
    ...
    def test_independant_de_la_cadence(self):
        ...
        for cadence in (30, 60, 120):
            ...
            for _ in range(cadence):  # exactement une seconde
                instant += 1 / cadence
                sortie = s.apply({"speed_kmh": 100.0}, instant)
            resultats.append(sortie["speed_kmh" + SUFFIXE])
    ...
    def test_bruit_fortement_reduit(self):
        ...
        for i in range(180):
            ...
            lisses.append(sortie["slip_max" + SUFFIXE])
...
class TestGardeFous(unittest.TestCase):
    ...
    def test_valeurs_non_numeriques_traversent(self):
        ...
        self.assertNotIn("car_name" + SUFFIXE, sortie)
    ...
    def test_non_fini_ne_contamine_pas_l_etat(self):
        ...
        self.assertNotIn("speed_kmh" + SUFFIXE, sortie)

        suite = s.apply({"speed_kmh": 50.0}, 0.2)
        self.assertTrue(math.isfinite(suite["speed_kmh" + SUFFIXE]))
        self.assertAlmostEqual(suite["speed_kmh" + SUFFIXE], 50.0, places=6)
    ...
    def test_ecart_de_temps_nul_ou_negatif(self):
        ...
        self.assertEqual(sortie["speed_kmh" + SUFFIXE], 20.0)
...
class TestReset(unittest.TestCase):
    def test_reset_repart_de_la_nouvelle_valeur(self):
        ...
        s.reset()
        self.assertEqual(s.apply({"speed_kmh": 0.0}, 0.2)["speed_kmh" + SUFFIXE], 0.0)
    ...
    def test_configure_oublie_les_canaux_retires(self):
        ...
        self.assertEqual(s.reglages, {"speed_kmh": 0.2})
    ...
    def test_contient_les_entiers_significatifs(self):
        for nom in ("gear", "lap_number", "car_ordinal", "is_race_on"):
            with self.subTest(canal=nom):
                self.assertIn(nom, NON_LISSABLES)
