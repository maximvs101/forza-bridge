...
from osc_targets import (CIBLE_PAR_DEFAUT, CibleInvalide, formate_cible,
                        formate_cibles, parse_cible, parse_cibles)


class TestFormesValides(unittest.TestCase):
    def test_ipv4(self):
        self.assertEqual(parse_cible("127.0.0.1:7000"), ("127.0.0.1", 7000))

    def test_nom_d_hote(self):
        self.assertEqual(parse_cible("studio-pc:9000"), ("studio-pc", 9000))

    def test_espaces_tolerees(self):
        self.assertEqual(parse_cible("  127.0.0.1 : 7000  "), ("127.0.0.1", 7000))

    def test_ipv6_entre_crochets(self):
        self.assertEqual(parse_cible("[::1]:7000"), ("::1", 7000))
        self.assertEqual(parse_cible("[fe80::1]:9000"), ("fe80::1", 9000))

    def test_bornes_de_port(self):
        self.assertEqual(parse_cible("h:1")[1], 1)
        self.assertEqual(parse_cible("h:65535")[1], 65535)
...
class TestFormesRefusees(unittest.TestCase):
    def test_hote_vide(self):
        ...
        with self.assertRaises(CibleInvalide):
            parse_cible(":7000")

    def test_port_manquant(self):
        with self.assertRaises(CibleInvalide):
            parse_cible("127.0.0.1")

    def test_port_non_numerique(self):
        with self.assertRaises(CibleInvalide):
            parse_cible("127.0.0.1:abc")
    ...
    def test_message_ne_suggere_pas_un_decoupage_faux(self):
        ...
        decoupage (\"[:]:1\" pour \"::1\")."""
        with self.assertRaises(CibleInvalide) as contexte:
            parse_cible("::1")
...
class TestListes(unittest.TestCase):
    def test_plusieurs_destinations(self):
        self.assertEqual(parse_cibles("127.0.0.1:7000, 192.168.0.50:9000"),
                         [("127.0.0.1", 7000), ("192.168.0.50", 9000)])

    def test_point_virgule_accepte(self):
        self.assertEqual(len(parse_cibles("a:1; b:2")), 2)
    ...
    def test_doublons_retires(self):
        ...
        self.assertEqual(parse_cibles("127.0.0.1:7000, 127.0.0.1:7000"),
                         [("127.0.0.1", 7000)])

    def test_ordre_conserve(self):
        cibles = parse_cibles("c:3, a:1, b:2")
    ...
    def test_une_entree_invalide_rejette_tout(self):
        with self.assertRaises(CibleInvalide):
            parse_cibles("127.0.0.1:7000, :9000")
...
class TestMiseEnForme(unittest.TestCase):
    def test_aller_retour(self):
        ...
        for texte in ("127.0.0.1:7000",
                      ...
                      "[::1]:7000, studio:9000"):
            with self.subTest(texte=texte):
                cibles = parse_cibles(texte)
                self.assertEqual(parse_cibles(formate_cibles(cibles)), cibles)

    def test_ipv6_remis_entre_crochets(self):
        self.assertEqual(formate_cible(("::1", 7000)), "[::1]:7000")

    def test_ipv4_sans_crochets(self):
        self.assertEqual(formate_cible(("127.0.0.1", 7000)), "127.0.0.1:7000")

    def test_defaut_analysable(self):
        self.assertEqual(parse_cible(formate_cible(CIBLE_PAR_DEFAUT)),
                         CIBLE_PAR_DEFAUT)
